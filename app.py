...
def main():
    ...
    with tabs[4]:
        ...
        if report:
            edge     = report.get("edge","No Edge")
            matrix   = report.get("matrix","?")
            up_pct   = report.get("up_pct",33)
            dn_pct   = report.get("down_pct",33)
            rb_pct   = report.get("range_pct",33)
        ...
        if today_b:
            ...
            for e in events_b[:20]:
                ...
                if bt not in pattern_costs:
                    pattern_costs[bt] = \
                        {"count":0,"cost":0}
                pattern_costs[bt]["count"] += 1
                pattern_costs[bt]["cost"] += cost
