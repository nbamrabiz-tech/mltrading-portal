...
def main():
    ...
    with tabs[4]:
        ...
        try:
            with engine.connect() as conn:
                accounts = conn.execute(text("""
                    SELECT
                        ...
                        s.max_loss_multiplier,
                        s.is_active,
                        COALESCE(b.opening_balance,
                            lb.closing_balance,
                            s.current_balance) as bal,
                        COALESCE(b.risk_per_trade,
                            ROUND(COALESCE(
                                lb.closing_balance,
                                s.current_balance)
                                * s.risk_pct/100,2)
                            ) as risk,
                        COALESCE(b.max_loss_today,
                            ROUND(COALESCE(
                                lb.closing_balance,
                                s.current_balance)
                                * s.risk_pct/100
                    ...
                    LEFT JOIN account_balances b
                        ...
                        AND b.account_name =
                            s.account_name
                        AND b.balance_date = :td
                    LEFT JOIN LATERAL (
                        SELECT closing_balance
                        FROM account_balances lb2
                        WHERE lb2.market = s.market
                        AND lb2.account_name =
                            s.account_name
                ...
                """), {
                    "td": str(datetime.now(EST).date())
                }).fetchall()

                # Get today's P&L per account
            ...
            if accounts:
                ...
                for a in accounts:
                    name    = a[0]
                    atype   = a[1]
                    balance = float(a[5] or 0)
                    risk    = float(a[6] or 0)
                    max_loss= float(a[7] or 0)

                    pnl_today = pnl_map.get(
                        atype, pnl_map.get(
                            name, 0))
        ...
        with col_brief:
            ...
            # Consolidated behavioral section
            st.markdown(
                "**📋 Behavioral Patterns "
                "— Last 30 Days**")
            ...
            if streaks:
                ...
                if ts > 0 and tt:
                    ...
                    color = ("#1B5E20" if tt=="Win"
                             else "#CC0000")
                    s1.markdown(
                        f"<div style='text-align:"
                        f"center;padding:8px;'>"
                        f"<div style='font-size:20px;"
                        f"'>{icon}</div>"
                        f"<div style='color:{color};"
                        f"font-weight:600;"
                        f"font-size:13px;'>"
                        f"{ts} {tt}s in a row</div>"
                ...
                if ps > 0 and pt:
                    ...
                    s2.markdown(
                        ...
                        f"font-weight:600;"
                        f"font-size:13px;'>"
                        f"{ps} {pt} in a row</div>"
                        f"<div style='color:#888;"
                        f"font-size:10px;'>"
                        f"Prediction streak</div>"
                        f"</div>",
                        unsafe_allow_html=True)
        ...
        if trades:
            ...
            for t in trades[:20]:
                ...
                # Exit type display
                exit_type = t[21] if len(t)>21 else None
                exit_reason = t[22] if len(t)>22 else None
                exit_reason_type = t[23] \
                    if len(t)>23 else None
                post_exit = t[24] if len(t)>24 else None

                exit_txt = ""
                if exit_type == "Conviction Exit":
                    rtype = exit_reason_type or ""
                    exit_txt = (
                        f"• 🎯 Conviction Exit "
                        f"({rtype})")
                elif exit_type == "Stop Loss":
                    exit_txt = "• 🛑 Stop Loss"
                elif exit_type == "Target Hit":
                    exit_txt = "• ✅ Target Hit"
                elif exit_type == "Breakeven Stop":
                    exit_txt = "• 🔒 Breakeven Stop"
                elif exit_type == "End of Day":
                    exit_txt = "• 🕐 End of Day"
    ...
    with tabs[5]:
        ...
        if total_trades < 10:
            st.info(
                f"📈 Analytics unlock as you trade.\n\n"
                f"Current: {total_trades} trades\n\n"
                f"🔓 Time analysis: 20+ trades\n"
                f"🔓 Setup performance: 10+ trades\n"
                f"🔓 Emotional correlation: 20+ trades\n"
                f"🔓 Day of week: 30+ trades\n\n"
                f"Keep logging every trade. "
                f"The insights will appear automatically."
            )
        else:
            # Setup performance
            ...
            if setup_data:
                ...
                df = pd.DataFrame(setup_data,
                    columns=["Setup","Trades",
                             "Win Rate","P&L"])
                df["Win Rate"] = df["Win Rate"]                    .apply(lambda x: f"{x:.0f}%")
