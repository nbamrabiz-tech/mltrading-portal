...
def main():
    ...
    with tabs[4]:
        ...
        # ── Behavioral patterns ──────────
        # ── Behavioral patterns redesigned ────
        st.markdown(
        ...
        PLAIN_BEHAVIOR = {
            ...
            "Wrong Setup for Day": (
                ...
                "🟡","#FFF3CD",False,
                "match setup to today's edge"),
            "Respected No Edge": (
                "sat out when system said no edge",
                "✅","#E8F5E9",True,
                "keep it up"),
        }
        ...
        week_ago  = today_est - timedelta(days=7)
        ...
        yesterday_events = get_events_for_period(
            yesterday, yesterday)
        week_events = get_events_for_period(
            week_ago, today_est)
        month_events = events_b  # already loaded

        def render_period(label, evts,
                      show_empty=False):
            neg = []
            pos = []
            for e in evts:
                ...
                count = int(e[1])
                cost  = float(e[2] or 0)
                p = PLAIN_BEHAVIOR.get(btype)
                if not p:
                    continue
                desc,emoji,bg,is_pos,fix = p
                if is_pos:
                    pos.append(
                        (desc,count,cost,
                         emoji,bg,fix))
            ...
            st.markdown(
                ...
                f"{label}</div>",
                unsafe_allow_html=True)
            for desc,count,cost,                emoji,bg,fix in neg:
                cost_txt = (
                f" — cost "
                f"${abs(cost):.0f}"
                if cost < -5 else "")
                ...
                f"</div>",
                unsafe_allow_html=True)
            for desc,count,cost,                emoji,bg,fix in pos:
                st.markdown(
                f"<div style='"
                f"background:{bg};"
                f"padding:14px 18px;"
        ...
        # Trend indicator
        try:
            prev_week = today_est -                 timedelta(days=14)
            prev_events = get_events_for_period(
                prev_week, week_ago)
            this_neg = sum(
            ...
            prev_neg = sum(
                ...
                if e[0] not in
                ["Respected No Edge"])
            if this_neg < prev_neg:
                st.markdown(
                "<div style='background:"
                "#E8F5E9;padding:8px 12px;"
                "border-radius:6px;"
                "margin-top:8px;'>"
                "📈 <b>Improving</b> — "
                ...
                "vs last week.</div>",
                unsafe_allow_html=True)
            elif this_neg > prev_neg:
                st.markdown(
                "<div style='background:"
                "#FFE8E8;padding:8px 12px;"
                "border-radius:6px;"
                "margin-top:8px;'>"
                "📉 <b>More issues</b> "
        ...
        if month_events:
            ...
            pos_patterns = [
                e for e in month_events
                if e[0] == "Respected No Edge"]
            top_neg = neg_patterns[0]                 if neg_patterns else None
            top_pos = pos_patterns[0]                 if pos_patterns else None

            conclusion = ""
            if top_neg and top_pos:
                neg_desc = PLAIN_BEHAVIOR.get(
                top_neg[0],
                (top_neg[0],"","","",
                 ""))[0]
                conclusion = (
                f"Keep sitting out on No "
                f"Edge days — you do this "
                f"well. Work on not trading "
                f"when the system says sit "
                f"out.")
            elif top_neg:
                neg_desc = PLAIN_BEHAVIOR.get(
                top_neg[0],
                ...
                conclusion = (
                f"No major issues this "
                f"month. Discipline is "
                f"strong — keep it up.")
        ...
        try:
            ...
            with engine.connect() as conn:
                ...
                # Get ALL P&L per account
                # from inception to calculate
                # live balance
                ...
                # Today's P&L for used calculation
