...
def main():
            ...
            if negative:
                for desc, count, cost, \
                    ...
                    st.markdown(
                        ...
                        f"{count} times{cost_txt}."
                        f"</div>",
                        unsafe_allow_html=True)

            # Show positive separately
            if positive:
                for desc, count, cost, \
                        emoji, bg in positive:
                    st.markdown(
                        f"<div style='background:{bg};"
                        f"padding:8px 12px;"
                        f"border-radius:6px;"
    ...
    with tabs[5]:
        ...
        if tod and len(tod) >= 2:
            ...
            st.info(
                "Need 5+ trades across different "
                "time windows for this analysis.")
