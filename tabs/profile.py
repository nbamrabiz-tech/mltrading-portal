...
def render(engine, today, now_est):
    ...
    if report and _username == "sunny":
        ...
        if "No Edge" in str(edge):
            setup_icon = "⚪"
            setup_reminder = (
                "No Edge today — price action "
                "decides your setup.\n\n"
                "Range: Consolidation → breakout "
                "with volume + delta → FVG entry.\n\n"
                "Trend: Swing H/L → pullback to "
                "EMA-21/34 or VWAP → confirmation "
                "candle → enter.")
        elif "Range" in str(edge):
            setup_icon = "↔️"
            setup_reminder = (
                "Range setup: Wait for consolidation "
                "→ breakout with volume + delta → "
                "FVG → enter at FVG fill. "
                "Stop: other side of FVG.")
