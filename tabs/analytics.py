...
def render(engine, **kwargs):
    ...
    # Fetch behavioral data for money leaks
    events_b, _, _, _ = get_behavioral_data()
    ...
    custom_start = None
    custom_end   = None
    if days_filter == "Custom range":
        dr1, dr2 = st.columns(2)
        custom_start = dr1.date_input(
            "From", key="custom_start")
        custom_end   = dr2.date_input(
            "To",   key="custom_end",
            value=date.today())
    ...
    if days_filter == "Custom range" and             custom_start and custom_end:
        trades_j = get_trade_journal(365)
    ...
    exp = get_expectancy()
    if exp:
        ew1,ew2,ew3,ew4,ew5 = st.columns(5)
        ew1.metric("Win Rate",
                   f"{exp['win_rate']:.1f}%")
        ew2.metric("Avg Win",
                   f"${exp['avg_win']:+.2f}")
