...
def render(engine, today, now_est,
    ...
    if is_no_edge:
        ...
        with st.expander(
            ...
            # No Edge = no assumption about direction
            # Trader must read price action first
            st.markdown(
...
def render(engine, today, now_est,
    ...
    if is_no_edge:
        ...
        with st.expander(
            ...
            # Detect market context from report
            up_pct    = int(report_now.get(
                "up_pct", 33) if report_now
                else 33)
            down_pct  = int(report_now.get(
                "down_pct", 33) if report_now
                else 33)
            range_pct = int(report_now.get(
                "range_pct", 34) if report_now
                else 34)

            # Determine dominant tendency
            dominant = max(
                [("range", range_pct),
                 ("trend", max(up_pct, down_pct))],
                key=lambda x: x[1])
            context = dominant[0]
            ...
            if context == "range":
                ...
                st.markdown(
                    "**📊 Range Setup Confluence**\n"
                    "Today leans range — "
                    "use range rules:")
                conf_items = [
                    "Consolidation identified "
                    "(price in tight range)",
                    "Breakout occurred with "
                    "volume increase",
                    "Volume delta confirms "
                    "direction",
                    "FVG created on breakout",
                    "Price pulled back to FVG",
                ]
                ...
                # TREND SETUP CHECKLIST
                direction = "uptrend"                     if up_pct > down_pct                     else "downtrend"
                st.markdown(
                    f"**📈 Trend Setup Confluence**\n"
                    f"Today leans {direction} — "
                    f"use trend rules:")
                conf_items = [
                    "Swing high/low established",
                    "Pullback to EMA-21, "
                    "EMA-34 or VWAP",
                    "Confirmation candle "
                    "at the level",
            ...
            st.caption(
                f"Max 50% normal size. "
                f"Need {threshold}/"
                f"{label_count} to override.")
